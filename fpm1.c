...
}chopstick_pair_monitor_t;

typedef struct {
  int seat_id;
  const char *name;
  PS_STATUS status;
  chopstick_pair_monitor_t *csp;
  f_eat_t f_eat;
  f_think_t f_think;
}philosopher_t;

static chopstick_t chop_sticks[MAX_SEAT];
static chopstick_pair_monitor_t csp_monitor[MAX_SEAT];
...
static void csp_monitor_init(chopstick_pair_monitor_t *cspm, int max)
{
  int i;
  int left_cs_id, right_cs_id;
  chopstick_pair_monitor_t *ptr_cspm = cspm;
  ...
  {
    ptr_cspm->seat_id = i;
    ptr_cspm->status = IDLE;
	ptr_cspm->f_pickup = pickup;
	ptr_cspm->f_putdown = putdown;
	...
	{
	  ptr_cspm->left = &chop_sticks[left_cs_id];
	  ptr_cspm->right = &chop_sticks[right_cs_id];
	}
	else
	{
	  ptr_cspm->left = &chop_sticks[right_cs_id];
	  ptr_cspm->right = &chop_sticks[left_cs_id];
	}
	ptr_cspm++;
  }
}
...
void philosophers_init(philosopher_t *ps, int max)
{
  ...
  csp_monitor_init(csp_monitor, MAX_SEAT);
  ...
  for(i=0; i<max; i++)
  {
	...
	ptr_philosopher->csp = &csp_monitor[i];
...
static void pickup(void *usr_data)
{
	chopstick_pair_monitor_t *ptr_cspm = (chopstick_pair_monitor_t *)usr_data;
	
	pthread_mutex_lock(&ptr_cspm->left->lock);
	ptr_cspm->left->status = BUSY;
    clear_eol(ptr_cspm->seat_id);
    print(ptr_cspm->seat_id, 24, "chopstick%d", ptr_cspm->left->id);
	
    pthread_mutex_lock(&ptr_cspm->right->lock);
	ptr_cspm->right->status = BUSY;
	
    print(ptr_cspm->seat_id, 24 + 12, "chopstick%d", ptr_cspm->right->id);
}
...
static void putdown(void *usr_data)
{
	...
	ptr_cspm->right->status = IDLE;
    pthread_mutex_unlock(&ptr_cspm->right->lock);
	ptr_cspm->left->status = IDLE;
    pthread_mutex_unlock(&ptr_cspm->left->lock);
}
...
static void eat(void *usr_data) {
	...
	ptr_philosopher->csp->f_pickup((void *)(ptr_philosopher->csp));
	...
	ptr_philosopher->csp->f_putdown((void *)(ptr_philosopher->csp));
	ptr_philosopher->status = THINKING;
}
